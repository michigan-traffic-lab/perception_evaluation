...
def veh_evaluation(data_dir, args, cfg):
    ...
    if args.latency == -1:
        ...
        for trial_id in cfg['LATENCY_TRIAL_IDS']:
            det_traj_file = get_detection_file_path(args.system, data_dir, trial_id)
            ...
            vehicle_data = CAVData([cav_1_rtk_file, cav_1_speed_file], plot_name='CAV 1 trajectory recorded by RTK',
                                   gps_type='oxford')
            detection_data = DetectionData(det_traj_file, plot_name='detection system 1', detection_type=args.system)
    ...
    for trial_id in cfg['SINGLE_VEH_TRIAL_IDS']:
        ...
        evaluator = Evaluator(args.system, 'veh', 1.5, detection_data, vehicle_data, latency=args.latency,
                              ...
                              roi_radius=50)
        fp, fn, det_freq, lat_error, lon_error, id_switch, id_consistency, mota = evaluator.evaluate(visualize=args.visualize)
        fp_list.append(fp)
        fn_list.append(fn)
    ...
    for trial_id in cfg['TWO_VEH_TRIAL_IDS']:
        for veh in ['veh1', 'veh2']:
            det_traj_file = get_detection_file_path(args.system, data_dir, trial_id, veh)
...
def ped_evaluation(data_dir, args, cfg):
    ...
    for trial_id in cfg['PED_TRIAL_IDS']:
        ped_det_file_path = get_detection_file_path(args.system, data_dir, trial_id)
        ...
        print(f'++++++++++++++++++ For trip {trial_id}: Pedestrian +++++++++++++++++++')
        ## evaluate accuracy
